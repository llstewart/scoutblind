...
export async function POST(request: NextRequest) {
  ...
  analysisLogger.info(
    ...
    'Credits deducted for stream analysis'
  );
  ...
  try {
    ...
    analysisLogger.info({ jobId, totalBusinesses: businesses.length }, 'Workflow triggered for stream analysis');
    ...
    try {
      await import('@/lib/credits').then(m =>
        m.refundCredits(user.id, requestedCount,
          `Refund: job creation failed for "${niche}" in "${location}"`)
      );
    } catch (refundError) {
