...
export function BillingModal({ isOpen, onClose, currentTier, creditsRemaining }: BillingModalProps) {
  ...
  const tierName = SUBSCRIPTION_TIERS[currentTier as keyof typeof SUBSCRIPTION_TIERS]?.name || 'Free';
  ...
  return (
    <Modal
      ...
      subtitle={`${tierName} · ${displayCredits} credits`}
      size="full"
    >
      {/* Tabs */}
                    ...
                    <button
                      ...
                      className={`w-full py-2.5 px-4 rounded-lg font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
                        isCurrentTier
                          ...
                          : isPopular
                          ? 'bg-violet-600 hover:bg-violet-500 text-white active:scale-[0.97]'
